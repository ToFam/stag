...
double customEllipse::GetCenterX()
{
	return cX;
}
...
double customEllipse::GetSemiMajorAxis()
{
	return a;
}
double customEllipse::GetSemiMinorAxis()
{
	return b;
}

double customEllipse::GetRotation()
{
	return rotation;
}
...
inline double customEllipse::DistanceBwPoints(double x1, double y1, double x2, double y2)
{
...
inline double customEllipse::SquaredDistanceBwPoints(double x1, double y1, double x2, double y2)
{
	double xDist, yDist, distance;
...
inline double customEllipse::NewtonRaphsonIteration(double theta, double x, double y)
{
...
double customEllipse::NewtonRaphsonThetaEstimation(double th0, double x, double y)
{
...
double customEllipse::GetDistance(double pX, double pY, double& estimation)
{
	...
	for(int i=0 ; i<4 ; i++)
	{
		...
		distances[i] = DistanceBwPoints(transPntX, transPntY, candidatesX[i], candidatesY[i]);
...
double customEllipse::GetSquaredDistance(double pX, double pY, double& estimation)
{
	//for different initial values for 4 estimations (in radians)
	double th0[4], estimations[4];
	...
	for(int i=0 ; i<4 ; i++)
	{
		...
		distances[i] = SquaredDistanceBwPoints(transPntX, transPntY, candidatesX[i], candidatesY[i]);
...
double customEllipse::GetClosestPointAndDistance(double testX, double testY, pix &closest)
{
	double distance, estimation;
...
bool CircleFit(const vector<double> &Xs, const vector<double> &Ys, double &centerX, double &centerY, double &radius)
{
	...
	// radius is 0.4, if we tolerate 5% deviation, MSE must be at most 0.4 * 0.05 = 0.02
	// NOTE: I changed this 0.02 -> 0.0002, because observed deviation smaller than expected
	if (error > 0.0002)
		return false;
