...
void PoseRefiner::refineMarkerPose(EDInterface* edInterface, Marker& marker)
{
	...
	// to test this, we accumulate the min distances from the backprojected edge segment to 36 uniformly sampled points on the circular border
	...
	// it is a loop, the distance between the start and end points is below loopThres
	double loopThres = 7;
	// we sample the distances from the circular border on the marker plane to the backprojected edge segment
	// this distances will be referred to as error
	// the edge segment we are looking for is at most errorThres distant from the circular border
	double errorThres = 0.1;
	// the accumulated distances from the 36 points to the edge segment is at most accErrorThres
	...
	for (int indEdgeSegment = 0; indEdgeSegment < edgeMap->noSegments; indEdgeSegment++)
	{
		...
		for (int indEdgePix = 0; indEdgePix < edgeMap->segments[indEdgeSegment].noPixels; indEdgePix += minEdgeSegmentLength)
		{
			if (!checkIfPointInQuad(marker, Point2d(edgeMap->segments[indEdgeSegment].pixels[indEdgePix].c, edgeMap->segments[indEdgeSegment].pixels[indEdgePix].r)))
			{
		...
		vector<double> sampleErrors(36, INFINITY); // min distance from the 36 points to the edge segment
		vector<double> pixelErrors(edgeMap->segments[indEdgeSegment].noPixels, INFINITY); // min distance from edge segment individual edge segment pixels to the 36 points
		for (int indEdgePix = 0; indEdgePix < edgeMap->segments[indEdgeSegment].noPixels; indEdgePix++)
		{
	...
	for (int indEdgePix = 0; indEdgePix < ellipseEdgePixels.size(); indEdgePix++)
	{
		pix p;
...
bool PoseRefiner::checkIfPointInQuad(const Marker& marker, const Point2d& p)
{
	...
	c1c2 = Point2d(marker.corners[1].x - marker.corners[0].x, marker.corners[1].y - marker.corners[0].y);
	c1c4 = Point2d(marker.corners[3].x - marker.corners[0].x, marker.corners[3].y - marker.corners[0].y);
	c3c2 = Point2d(marker.corners[1].x - marker.corners[2].x, marker.corners[1].y - marker.corners[2].y);
	c3c4 = Point2d(marker.corners[3].x - marker.corners[2].x, marker.corners[3].y - marker.corners[2].y);

	c1p = Point2d(p.x - marker.corners[0].x, p.y - marker.corners[0].y);
	c3p = Point2d(p.x - marker.corners[2].x, p.y - marker.corners[2].y);

	if (crossProduct(c1p, c1c2) * crossProduct(c1p, c1c4) >= 0)
		return false;
	if (crossProduct(c1c2, c1p) * crossProduct(c1c2, c1c4) <= 0)
		return false;

	if (crossProduct(c3p, c3c2) * crossProduct(c3p, c3c4) >= 0)
		return false;
	if (crossProduct(c3c2, c3p) * crossProduct(c3c2, c3c4) <= 0)
		return false;
